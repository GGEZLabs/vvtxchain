...
func (s *IntegrationTestSuite) testStaking() {
	...
	delegation := sdk.NewCoin(uvvtxDenom, delegationAmount) // 500 vvtx

	// Alice delegate uvvtx to Validator A
	s.execDelegate(s.chainA, 0, delegation.String(), validatorAddressA, delegatorAddress.String(), vvtxHomePath, fees.String())
	...
	redelegation := sdk.NewCoin(uvvtxDenom, redelegationAmount) // 250 vvtx

	// Alice re-delegate half of her uvvtx delegation from Validator A to Validator B
	s.execRedelegate(s.chainA, 0, redelegation.String(), validatorAddressA, validatorAddressB, delegatorAddress.String(), vvtxHomePath, fees.String())
	...
	s.Require().Eventually(
		func() bool {
			...
			currDelegation = sdk.NewCoin(uvvtxDenom, currDelegationAmount)
	...
	// Alice unbonds all her uvvtx delegation from Validator A
	s.execUnbondDelegation(s.chainA, 0, currDelegation.String(), validatorAddressA, delegatorAddress.String(), vvtxHomePath, fees.String())
