...
import (
	...
	acltypes "github.com/GGEZLabs/vvtxchain/x/acl/types"
...
func (s *IntegrationTestSuite) execEncode(
	...
	s.T().Logf("%s - Executing vvtxchaind encoding with %v", c.id, txPath)
	vvtxCommand := []string{
		vvtxchaidBinary,
		txCommand,
		"encode",
	...
	for flag, value := range opts {
		vvtxCommand = append(vvtxCommand, fmt.Sprintf("--%s=%v", flag, value))
	}

	var encoded string
	s.executeTxCommand(ctx, c, vvtxCommand, 0, func(stdOut []byte, stdErr []byte) bool {
		if stdErr != nil {
			return false
		}
...
func (s *IntegrationTestSuite) execDecode(
	...
	s.T().Logf("%s - Executing vvtxchaind decoding with %v", c.id, txPath)
	vvtxCommand := []string{
		vvtxchaidBinary,
		txCommand,
		"decode",
...
func (s *IntegrationTestSuite) execVestingTx(
	...
	s.T().Logf("%s - Executing vvtxchaind %s with %v", c.id, method, args)
	vvtxCommand := []string{
		vvtxchaidBinary,
		txCommand,
	...
	vvtxCommand = append(vvtxCommand, args...)

	for flag, value := range opts {
		vvtxCommand = append(vvtxCommand, fmt.Sprintf("--%s=%v", flag, value))
	}

	s.executeTxCommand(ctx, c, vvtxCommand, 0, s.defaultExecValidation(c, 0))
...
func (s *IntegrationTestSuite) execCreatePeriodicVestingAccount(
	...
	s.T().Logf("Executing vvtxchaind create periodic vesting account %s", c.id)
...
func (s *IntegrationTestSuite) execUnjail(
	...
	s.T().Logf("Executing vvtxchaind slashing unjail %s with options: %v", c.id, opt)
	vvtxCommand := []string{
		vvtxchaidBinary,
		txCommand,
...
func (s *IntegrationTestSuite) execFeeGrant(c *chain, valIdx int, granter, grantee, spendLimit string, opt ...flagOption) {
	...
	for flag, value := range opts {
		vvtxCommand = append(vvtxCommand, fmt.Sprintf("--%s=%s", flag, value))
	}
	s.T().Logf("running feegrant on chain: %s - Tx %v", c.id, vvtxCommand)

	s.executeTxCommand(ctx, c, vvtxCommand, valIdx, s.defaultExecValidation(c, valIdx))
}
...
func (s *IntegrationTestSuite) execBankSend(
	...
	s.executeTxCommand(ctx, c, vvtxCommand, valIdx, s.expectErrExecValidation(c, valIdx, expectErr))
}
...
func (s *IntegrationTestSuite) execBankMultiSend(
	...
	vvtxCommand = append(vvtxCommand, to...)
	vvtxCommand = append(vvtxCommand, amt, "-y")
...
func (s *IntegrationTestSuite) execProtocolpoolFundCommunityPool(c *chain, valIdx int, from, amt, fees string) {
	...
	s.T().Logf("Executing vvtxchaind tx protocolpool fund-community-pool on chain %s", c.id)
...
func (s *IntegrationTestSuite) runGovExec(c *chain, valIdx int, submitterAddr, govCommand string, proposalFlags []string, fees string, validationFunc func([]byte, []byte) bool) {
	...
	vvtxCommand = concatFlags(vvtxCommand, proposalFlags, generalFlags)
	s.T().Logf("Executing vvtxchaind tx gov %s on chain %s", govCommand, c.id)
	s.executeTxCommand(ctx, c, vvtxCommand, valIdx, validateResponse)
...
// 	s.executeTxCommand(ctx, c, vvtxCommand, valIdx, func(stdOut []byte, stdErr []byte) bool {
// 		// vvtxchaind keys add by default returns payload to stdErr
...
// 	s.executeTxCommand(ctx, c, vvtxCommand, valIdx, func([]byte, []byte) bool {
// 		return true
// 	})
// }
...
func (s *IntegrationTestSuite) execDelegate(c *chain, valIdx int, amount, valOperAddress, delegatorAddr, home, delegateFees string) { //nolint:unparam
	...
	s.T().Logf("Executing vvtxchaind tx staking delegate %s", c.id)
...
func (s *IntegrationTestSuite) execUnbondDelegation(c *chain, valIdx int, amount, valOperAddress, delegatorAddr, home, delegateFees string) {
	...
	s.T().Logf("Executing vvtxchaind tx staking unbond %s", c.id)
...
func (s *IntegrationTestSuite) execCancelUnbondingDelegation(c *chain, valIdx int, amount, valOperAddress, creationHeight, delegatorAddr, home, delegateFees string) {
	...
	s.T().Logf("Executing vvtxchaind tx staking cancel-unbond %s", c.id)
...
func (s *IntegrationTestSuite) execRedelegate(c *chain, valIdx int, amount, originalValOperAddress,
	...
	s.T().Logf("Executing vvtxchaind tx staking redelegate %s", c.id)
...
func (s *IntegrationTestSuite) getLatestBlockHeight(c *chain, valIdx int) int {
	...
	vvtxCommand := []string{vvtxchaidBinary, "status"}
	s.executeTxCommand(ctx, c, vvtxCommand, valIdx, func(stdOut []byte, stdErr []byte) bool {
		var (
			err   error
			block syncInfo
...
// 		func() bool {
// 			afterOmBalance, err := getSpecificBalance(endpoint, recipientAddress, uvvtxDenom)
// 			s.Require().NoError(err)
...
func (s *IntegrationTestSuite) execWithdrawReward(
	...
	vvtxCommand := []string{
		...
		validatorAddress,
		fmt.Sprintf("--%s=%s", flags.FlagFrom, delegatorAddress),
		fmt.Sprintf("--%s=%s", flags.FlagGasPrices, "300uvvtx"),
...
func (s *IntegrationTestSuite) executeTxCommand(ctx context.Context, c *chain, vvtxCommand []string, valIdx int, validation func([]byte, []byte) bool) {
	if validation == nil {
...
// 	s.T().Logf("Executing vvtxchaind tx staking validator-bond %s", c.id)
...
// 	s.T().Logf("Executing vvtxchaind tx staking tokenize-share %s", c.id)
...
func (s *IntegrationTestSuite) executeRedeemShares(c *chain, valIdx int, amount, delegatorAddr, home, delegateFees string) {
	...
	s.T().Logf("Executing vvtxchaind tx staking redeem-tokens %s", c.id)
...
func (s *IntegrationTestSuite) executeTransferTokenizeShareRecord(c *chain, valIdx int, recordID, owner, newOwner, home, txFees string) {
	...
	s.T().Logf("Executing vvtxchaind tx staking transfer-tokenize-share-record %s", c.id)
...
// signTxFileOnline signs a transaction file using the vvtxcli tx sign command
...
func (s *IntegrationTestSuite) signTxFileOnline(chain *chain, valIdx int, from string, txFilePath string) ([]byte, error) {
	...
	vvtxCommand := []string{
		...
		txCommand,
		"sign",
		filepath.Join(vvtxHomePath, txFilePath),
		fmt.Sprintf("--%s=%s", flags.FlagChainID, chain.id),
		fmt.Sprintf("--%s=%s", flags.FlagHome, vvtxHomePath),
		fmt.Sprintf("--%s=%s", flags.FlagFrom, from),
		"--keyring-backend=test",
		"--output=json",
		"-y",
	...
	s.executeTxCommand(ctx, chain, vvtxCommand, valIdx, captureOutput)
	if len(erroutput) > 0 {
...
// broadcastTxFile broadcasts a signed transaction file using the vvtxcli tx broadcast command
// the from flag is used to specify the keyring account to sign the transaction
// the from account must be registered in the keyring and exist on chain (have a balance or be a genesis account)
//
//nolint:unused
...
func (s *IntegrationTestSuite) execInitAcl(c *chain, valIdx int, superAdmin, from, home, initFees string) {
	...
	s.T().Logf("Executing vvtxchaind tx acl init %s", c.id)
...
func (s *IntegrationTestSuite) execAddAclAdmin(c *chain, valIdx int, admin, from, home, addAdminFees string) {
	...
	s.T().Logf("Executing vvtxchaind tx acl add-admin %s", c.id)
...
func (s *IntegrationTestSuite) execDeleteAclAdmin(c *chain, valIdx int, admin, from, home, deleteAdminFees string) {
	...
	s.T().Logf("Executing vvtxchaind tx acl delete-admin %s", c.id)
...
func (s *IntegrationTestSuite) execAddAclAuthority(c *chain, valIdx int,
	...
	s.T().Logf("Executing vvtxchaind tx acl add-authority %s", c.id)
...
func (s *IntegrationTestSuite) execDeleteAclAuthority(c *chain, valIdx int, authAddress, from, home, deleteAuthFees string) {
	...
	s.T().Logf("Executing vvtxchaind tx acl delete-authority %s", c.id)
...
func (s *IntegrationTestSuite) execUpdateAclAuthority(c *chain, valIdx int, authAddress, deleteAccessDefinitions, updateAccessDefinition, from, home, deleteAuthFees string) {
	...
	s.T().Logf("Executing vvtxchaind tx acl update-authority %s", c.id)
...
func (s *IntegrationTestSuite) execCreateTrade(c *chain, valIdx int,
	...
	s.T().Logf("Executing vvtxchaind tx trade create-trade %s", c.id)
...
func (s *IntegrationTestSuite) execProcessTrade(c *chain, valIdx int,
	...
	s.T().Logf("Executing vvtxchaind tx trade process-trade %s", c.id)
	...
	if expErr {
		s.executeTxCommand(ctx, c, vvtxCommand, valIdx, s.expectErrExecValidation(c, valIdx, true))
