...
const (
	vvtxchaidBinary = "vvtxchaind"
	txCommand       = "tx"
	queryCommand    = "query"
	keysCommand     = "keys"
	vvtxHomePath    = "/home/nonroot/.vvtxchain"
	uvvtxDenom      = "uvvtx"
	initBalanceStr  = "100000000000000000uvvtx"
	minGasPrice     = "0.01"
	...
	govAuthority = "vvtx10d07y265gmmuvt4z0w9aw880jnsr700jc85elf" //nolint:unused
)

var (
	vvtxConfigPath    = filepath.Join(vvtxHomePath, "config")
	stakingAmount     = math.NewInt(100000000000)
	stakingAmountCoin = sdk.NewCoin(uvvtxDenom, stakingAmount)
	tokenAmount       = sdk.NewCoin(uvvtxDenom, math.NewInt(3300000000))
	standardFees      = sdk.NewCoin(uvvtxDenom, math.NewInt(330000))
	depositAmount     = sdk.NewCoin(uvvtxDenom, math.NewInt(330000000))
...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	//
	// 1. Initialize vvtx validator nodes.
	// 2. Create and initialize vvtx validator genesis files (both chains)
...
func (s *IntegrationTestSuite) TearDownSuite() {
	if str := os.Getenv("GGEZ_E2E_SKIP_CLEANUP"); len(str) > 0 {
...
func (s *IntegrationTestSuite) initNodes(c *chain) {
	...
	*/
	...
	s.Require().NoError(
		modifyGenesis(val0ConfigDir, "", initBalanceStr, addrAll, uvvtxDenom),
	)
...
func (s *IntegrationTestSuite) addGenesisVestingAndJailedAccounts(
	...
	stakingModuleBalances := banktypes.Balance{
		...
		Coins:   sdk.NewCoins(sdk.NewCoin(uvvtxDenom, math.NewInt(slashingShares))),
	}
...
func (s *IntegrationTestSuite) initValidatorConfigs(c *chain) {
	for i, val := range c.validators {
		...
		appConfig.MinGasPrices = fmt.Sprintf("%s%s", minGasPrice, uvvtxDenom)
...
func (s *IntegrationTestSuite) runValidators(c *chain, portOffset int) {
	s.T().Logf("starting vvtx %s validator containers...", c.id)
	...
	for i, val := range c.validators {
		runOpts := &dockertest.RunOptions{
			...
			Mounts: []string{
				fmt.Sprintf("%s/:%s", val.configDir(), vvtxHomePath),
			},
			Repository: "ggezlabs/vvtxchain",
			Tag:        "latest",
		}
		...
		s.T().Logf("started vvtx %s validator container: %s", c.id, resource.Container.ID)
	}
	...
	s.Require().Eventually(
		...
		5*time.Minute,
		time.Second,
		"vvtx node failed to produce blocks",
	)
}
...
func (s *IntegrationTestSuite) runIBCRelayer() {
	...
	tmpDir, err := os.MkdirTemp("", "vvtx-e2e-testnet-hermes-")
	s.Require().NoError(err)
	s.tmpDirs = append(s.tmpDirs, tmpDir)

	vvtxAVal := s.chainA.validators[0]
	vvtxBVal := s.chainB.validators[0]

	vvtxARly := s.chainA.genesisAccounts[relayerAccountIndexHermes]
	vvtxBRly := s.chainB.genesisAccounts[relayerAccountIndexHermes]
	...
	s.hermesResource, err = s.dkrPool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				fmt.Sprintf("VVTX_A_E2E_CHAIN_ID=%s", s.chainA.id),
				fmt.Sprintf("VVTX_B_E2E_CHAIN_ID=%s", s.chainB.id),
				fmt.Sprintf("VVTX_A_E2E_VAL_MNEMONIC=%s", vvtxAVal.mnemonic),
				fmt.Sprintf("VVTX_B_E2E_VAL_MNEMONIC=%s", vvtxBVal.mnemonic),
				fmt.Sprintf("VVTX_A_E2E_RLY_MNEMONIC=%s", vvtxARly.mnemonic),
				fmt.Sprintf("VVTX_B_E2E_RLY_MNEMONIC=%s", vvtxBRly.mnemonic),
				fmt.Sprintf("VVTX_A_E2E_VAL_HOST=%s", s.valResources[s.chainA.id][0].Container.Name[1:]),
				fmt.Sprintf("VVTX_B_E2E_VAL_HOST=%s", s.valResources[s.chainB.id][0].Container.Name[1:]),
			},
			User: "root",
			Entrypoint: []string{
				"sh",
	...
	// create the client, connection and channel between the two vvtx chains
	s.createConnection()
	s.createChannel()
}
		...
		"messages": [
		 {
		  ...
		  "authority": "vvtx10d07y265gmmuvt4z0w9aw880jnsr700jc85elf",
		  "plan": {
		   "name": "%s",
		   "height": "%d",
		   "info": "test",
...
func configFile(filename string) string {
	filepath := filepath.Join(vvtxConfigPath, filename)
	return filepath
}
