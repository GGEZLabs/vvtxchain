...
func (s *IntegrationTestSuite) GovCommunityPoolSpend() {
	...
	sendAmount := sdk.NewCoin(uvvtxDenom, math.NewInt(10000000)) // 10uvvtx
	s.writeGovCommunitySpendProposal(s.chainA, sendAmount, recipient)

	beforeRecipientBalance, err := getSpecificBalance(chainAAPIEndpoint, recipient, uvvtxDenom)
	...
	s.Require().Eventually(
		func() bool {
			afterRecipientBalance, err := getSpecificBalance(chainAAPIEndpoint, recipient, uvvtxDenom)
			s.Require().NoError(err)
...
// you can use `vvtxchaind tx gov draft-proposal` to create a proposal file that you can use
// min initial deposit of 100uvvtx is required in e2e tests, otherwise the proposal would be dropped
