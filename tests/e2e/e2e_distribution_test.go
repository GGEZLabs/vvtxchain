...
func (s *IntegrationTestSuite) testDistribution() {
	...
	beforeBalance, err := getSpecificBalance(chainEndpoint, newWithdrawalAddress.String(), uvvtxDenom)
	s.Require().NoError(err)
	if beforeBalance.IsNil() {
		beforeBalance = sdk.NewCoin(uvvtxDenom, math.NewInt(0))
	}

	s.execSetWithdrawAddress(s.chainA, 0, standardFees.String(), delegatorAddress.String(), newWithdrawalAddress.String(), vvtxHomePath)
	...
	s.execWithdrawReward(s.chainA, 0, delegatorAddress.String(), valOperAddressA, vvtxHomePath)
	s.Require().Eventually(
		func() bool {
			afterBalance, err := getSpecificBalance(chainEndpoint, newWithdrawalAddress.String(), uvvtxDenom)
			s.Require().NoError(err)
...
func (s *IntegrationTestSuite) fundCommunityPool() {
	...
	beforeDistUVVTXBalance, _ := getSpecificBalance(chainAAPIEndpoint, distModuleAddress, tokenAmount.Denom)
	if beforeDistUVVTXBalance.IsNil() {
		// Set balance to 0 if previous balance does not exist
		beforeDistUVVTXBalance = sdk.NewInt64Coin(uvvtxDenom, 0)
	}
	...
	s.Require().Eventually(
		func() bool {
			afterDistUVVTXBalance, err := getSpecificBalance(chainAAPIEndpoint, distModuleAddress, tokenAmount.Denom)
			s.Require().NoErrorf(err, "Error getting balance: %s", afterDistUVVTXBalance)
			...
			return beforeDistUVVTXBalance.Add(tokenAmount).Add(standardFees).IsLT(afterDistUVVTXBalance)
		},
		15*time.Second,
		5*time.Second,
	)
