...
import (
	...
	vvtxchain "github.com/GGEZLabs/vvtxchain/app"
...
var (
	encodingConfig vvtxchain.EncodingConfig
	cdc            codec.Codec
	txConfig       client.TxConfig
)

func init() {
	encodingConfig = vvtxchain.MakeEncodingConfig()
...
func (c *chain) createAndInitValidators(count int) error {
	...
	appOptions[flags.FlagHome] = vvtxchain.DefaultNodeHome
	appOptions[server.FlagInvCheckPeriod] = simcli.FlagPeriodValue //nolint:staticcheck

	tempApplication := vvtxchain.New(
		log.NewNopLogger(),
		dbm.NewMemDB(),
		nil,
		true,
		appOptions,
		baseapp.SetChainID("vvtxchain-testnet"),
	)
	defer func() {
