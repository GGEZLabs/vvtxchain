...
var (
	...
	vestingAmountVested     = sdk.NewCoin(uvvtxDenom, math.NewInt(99900000000))
	vestingAmount           = sdk.NewCoin(uvvtxDenom, math.NewInt(350000))
	vestingBalance          = sdk.NewCoins(vestingAmountVested).Add(vestingAmount)
	vestingDelegationAmount = sdk.NewCoin(uvvtxDenom, math.NewInt(500000000))
	vestingDelegationFees   = sdk.NewCoin(uvvtxDenom, math.NewInt(1))
)
...
func (s *IntegrationTestSuite) testDelayedVestingAccount(api string) {
	...
	s.Run("test delayed vesting genesis account", func() {
		...
		balance, err := getSpecificBalance(api, vestingDelayedAcc.String(), uvvtxDenom)
		s.Require().NoError(err)
		s.Require().Equal(vestingBalance.AmountOf(uvvtxDenom), balance.Amount)

		// Delegate coins should succeed
		s.execDelegate(chain, valIdx, vestingDelegationAmount.String(), valOpAddr,
			vestingDelayedAcc.String(), vvtxHomePath, vestingDelegationFees.String())
...
func (s *IntegrationTestSuite) testContinuousVestingAccount(api string) {
	s.Run("test continuous vesting genesis account", func() {
		...
		//	Check address balance
		balance, err := getSpecificBalance(api, continuousVestingAcc.String(), uvvtxDenom)
		s.Require().NoError(err)
		s.Require().Equal(vestingBalance.AmountOf(uvvtxDenom), balance.Amount)

		// Delegate coins should succeed
		s.execDelegate(chain, valIdx, vestingDelegationAmount.String(),
			valOpAddr, continuousVestingAcc.String(), vvtxHomePath, vestingDelegationFees.String())
...
func (s *IntegrationTestSuite) testPeriodicVestingAccount(api string) {
	s.Run("test periodic vesting genesis account", func() {
		...
		s.execCreatePeriodicVestingAccount(
			chain,
			periodicVestingAddr,
			filepath.Join(vvtxHomePath, vestingPeriodFile),
		...
		//	Check address balance
		balance, err := getSpecificBalance(api, periodicVestingAddr, uvvtxDenom)
		s.Require().NoError(err)

		expectedBalance := sdk.NewCoin(uvvtxDenom, math.NewInt(0))
		for _, period := range acc.VestingPeriods {
			// _, coin := ante.Find(period.Amount, uvvtxDenom)
			_, coin := period.Amount.Find(uvvtxDenom)
		...
		// Delegate coins should succeed
		s.execDelegate(chain, valIdx, vestingDelegationAmount.String(), valOpAddr,
			periodicVestingAddr, vvtxHomePath, vestingDelegationFees.String())
...
func generateVestingPeriod() ([]byte, error) {
	p := vestingPeriod{
		...
		Periods: []period{
			{
				Coins:  "850000000" + uvvtxDenom,
				Length: 35,
			},
			{
				Coins:  "2000000000" + uvvtxDenom,
				Length: 35,
			},
		},
	}
