...
help:
	...
	@echo "    make [command]"
	@echo ""
	@echo "  make install               Install vvtxchaind binary"
	@echo "  make build                 Build vvtxchaind binary"
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=vvtxchain \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=vvtxchaind \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o $(OUTPUT_DIR)/vvtxchaind ./cmd/vvtxchaind
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o $(OUTPUT_DIR)/vvtxchaind.exe ./cmd/vvtxchaind

install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/vvtxchaind

build-image:
	docker build -f Dockerfile -t ggezlabs/vvtxchain .
...
set-testnet-configs:
	vvtxchaind config set client chain-id localchain_9000-1
	vvtxchaind config set client keyring-backend test
	vvtxchaind config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	@echo "Adding acc0..."
	@echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | vvtxchaind keys add acc0 --recover
	@echo "Adding acc1..."
	@echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | vvtxchaind keys add acc1 --recover
